...
const profilRCSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .prh-menu-btn:hover { background: var(--bg-page); color: var(--text-primary); }
  ...
  .profil-grid { display: grid; grid-template-columns: 310px 1fr; gap: 20px; align-items: start; }
  ...
  .avatar-wrap { position: relative; width: 130px; height: 130px; margin: 0 auto 20px; border-radius: 50%; background: linear-gradient(135deg, #e8f2fd, #d0e8fb); border: 3px solid rgba(41,128,232,0.15); overflow: hidden; cursor: pointer; transition: var(--tr); }
  ...
  .avatar-name { font-size: 20px; font-weight: 800; color: var(--text-primary); }
  ...
  .stats-mini { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-top: 16px; }
  .stat-mini-card { background: var(--bg-page); border-radius: 14px; padding: 14px; text-align: center; }
  .stat-mini-val   { font-size: 24px; font-weight: 800; color: var(--brand-dark); }
  .stat-mini-label { font-size: 10px; color: var(--text-muted); margin-top: 2px; }

  .banner-card { background: linear-gradient(135deg, var(--brand-dark) 0%, var(--brand-mid) 100%); border-radius: var(--radius-card); padding: 24px; color: #fff; position: relative; overflow: hidden; margin-top: 16px; min-height: 160px; display: flex; flex-direction: column; justify-content: space-between; border: none; box-shadow: var(--shadow-sm); transition: var(--tr); }
  ...
  .banner-desc { font-size: 12px; color: rgba(255,255,255,0.6); margin-top: 6px; }
  .banner-pill { display: inline-flex; align-items: center; gap: 6px; background: rgba(255,255,255,0.15); border: 1px solid rgba(255,255,255,0.2); color: #fff; font-size: 12px; font-weight: 700; padding: 6px 14px; border-radius: 20px; margin-top: 14px; width: fit-content; }

  .form-card { padding: 28px 28px 24px; }
  .form-title { font-size: 20px; font-weight: 800; color: var(--text-primary); margin-bottom: 4px; }
  ...
  .form-input.err:focus { box-shadow: 0 0 0 3px rgba(239,68,68,0.1); }
  ...
  @media (max-width: 1100px) { .profil-grid { grid-template-columns: 270px 1fr; } }
  ...
  @media (max-width: 480px) { .prc { padding: 12px; } .profil-grid { gap: 14px; } .avatar-wrap { width: 100px; height: 100px; } .form-actions { width: 100%; justify-content: flex-end; } .stats-mini { grid-template-columns: 1fr 1fr; } }
`;

if (typeof document !== "undefined" && !document.getElementById("airops-profilr-css")) {
  const tag = document.createElement("style");
  tag.id = "airops-profilr-css";
  tag.textContent = profilRCSS;
  document.head.appendChild(tag);
}

export const STORAGE_FORM_R  = "airops_responsable_form_v1";
export const STORAGE_PHOTO_R = "airops_responsable_photo_v1";

export function getStoredRNom()      { try { const s = localStorage.getItem(STORAGE_FORM_R); return s ? (JSON.parse(s).nom || "Ahmed Mansour") : "Ahmed Mansour"; } catch { return "Ahmed Mansour"; } }
export function getStoredRPhoto()    { try { return localStorage.getItem(STORAGE_PHOTO_R) || ""; } catch { return ""; } }
export function getStoredRInitials(nom) { return (nom||"AM").trim().split(" ").map(w=>w[0]).slice(0,2).join("").toUpperCase()||"AM"; }

const defaultForm = {
  nom:       "Ahmed Mansour",
  email:     "a.mansour@nouvelair.tn",
...
export default function ProfilR() {
  ...
  const [form, setForm] = useState(() => {
    try {
      const s = localStorage.getItem(STORAGE_FORM_R);
      if (s) {
        const parsed = JSON.parse(s);
        // Supprimer le champ poste s'il existe dans le localStorage
        const { poste: _p, password: _pw, ...rest } = parsed;
        return { ...defaultForm, ...rest, password: "" };
      }
      return defaultForm;
  ...
  /* Persist form (sans password ni poste) */
  useEffect(() => {
    try {
      const { password: _p, ...rest } = form;
      localStorage.setItem(STORAGE_FORM_R, JSON.stringify(rest));
      window.dispatchEvent(new Event("airops-responsable-update"));
    } catch {}
  }, [form]);
  ...
  useEffect(() => {
    try {
      if (photo) { localStorage.setItem(STORAGE_PHOTO_R, photo); }
      else        { localStorage.removeItem(STORAGE_PHOTO_R); }
      window.dispatchEvent(new Event("airops-responsable-update"));
    } catch {}
  }, [photo]);
  ...
  const handleReset = () => {
    try {
      const saved = localStorage.getItem(STORAGE_FORM_R);
      if (saved) {
        const { poste: _p, password: _pw, ...rest } = JSON.parse(saved);
        setForm({ ...defaultForm, ...rest, password: "" });
      } else {
        setForm(defaultForm);
      }
  ...
  const handlePhotoChange = e => {
    ...
    if (!file) return;
    if (!file.type.startsWith("image/")) { setToast("Veuillez choisir une image valide."); return; }
    if (file.size > 2 * 1024 * 1024)    { setToast("L'image ne doit pas dépasser 2 MB."); return; }
  ...
  const initials = getStoredRInitials(form.nom);
  const inputCls = field => `form-input${touched[field] && errors[field] ? " err" : ""}`;
      ...
      <div className="prm">
        <header className="prh">
          ...
          <div className="prh-right">
            <div className="user-chip">
              <span className="user-name-h">{form.nom || "Ahmed Mansour"}</span>
        ...
        <main className="prc">
          ...
          <div className="profil-grid">
            ...
            <div>
              <div className="card avatar-card">
                <div className="avatar-wrap" onClick={() => fileRef.current?.click()} title="Cliquer pour changer la photo">
                  {photo
                    ...
                    : <div className="avatar-placeholder">
                        <svg width="32" height="32" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M3 16l4-4a3 3 0 014.243 0L16 16m-2-2l1-1a3 3 0 014.243 0L21 15m-6-8h.01"/></svg>
                        <span style={{ fontSize: 11, marginTop: 5, fontWeight: 600 }}>Ajouter photo</span>
                      </div>
                  }
                  <div className="avatar-overlay">
                    <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="white" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z"/><path strokeLinecap="round" strokeLinejoin="round" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                ...
                <p className="avatar-name">{form.nom || "Ahmed Mansour"}</p>

                {/* Badge rôle fixe — sans "poste" */}
                ...
                <div className="info-row"><p className="info-row-label">Adresse</p><p className="info-row-value">{form.adresse || "—"}</p></div>

                <div className="stats-mini">
                  <div className="stat-mini-card">
                    <div className="stat-mini-val">14</div>
                    <div className="stat-mini-label">Chauffeurs</div>
                  </div>
                  <div className="stat-mini-card">
                    <div className="stat-mini-val">11</div>
              ...
              <div className="banner-card">
                <div className="banner-deco" style={{ width: 150, height: 150, right: -40, bottom: -40 }}/>
                <div className="banner-deco" style={{ width: 70, height: 70, right: 60, top: -20 }}/>
                <div>
                  <p className="banner-sub">Espace responsable</p>
                  <p className="banner-title">Gardez votre profil à jour</p>
                  <p className="banner-desc">Un profil complet assure une meilleure gestion des opérations.</p>
                </div>
            ...
            {/* Colonne droite — formulaire sans champ poste */}
            <div className="card form-card">
              <p className="form-title">Paramètres du compte</p>
              <p className="form-subtitle">Modifiez vos informations personnelles ci-dessous.</p>
              ...
              <form onSubmit={handleSubmit} noValidate>
                <div className="form-grid">
                  ...
                  <div className="form-group">
                    ...
                    <input type="text" name="nom" value={form.nom} onChange={handleChange} onBlur={handleBlur} className={inputCls("nom")} placeholder="Ahmed Mansour"/>
                  ...
                  <div className="form-group">
                    ...
                    <input type="email" name="email" value={form.email} onChange={handleChange} onBlur={handleBlur} className={inputCls("email")} placeholder="email@nouvelair.tn"/>
                  ...
                  {/* Nouveau mot de passe — full width */}
          ...
          <div style={{ fontSize: 10, color: "var(--text-muted)", textAlign: "center", padding: "20px 0 8px", letterSpacing: 1, textTransform: "uppercase" }}>
            © 2026 AirOps – Gestion Transport Responsable
          </div>
        </main>
      </div>
