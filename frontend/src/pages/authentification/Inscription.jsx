...
export default function Inscription() {
  ...
  const selectedPassport =
    PASSPORT_PREFIXES.find((p) => p.code === passportCountry) ||
    PASSPORT_PREFIXES[0];

  useEffect(() => {
    if (localStorage.getItem("in_nom")) setNom(localStorage.getItem("in_nom"));
    if (localStorage.getItem("in_email"))
      setEmail(localStorage.getItem("in_email"));
    if (localStorage.getItem("in_tel"))
      setTelephone(localStorage.getItem("in_tel"));
    if (localStorage.getItem("in_addr"))
      setAddress(localStorage.getItem("in_addr"));
    if (localStorage.getItem("in_nat"))
      setNationality(localStorage.getItem("in_nat"));
  }, []);

  useEffect(() => {
    localStorage.setItem("in_nom", nom);
  }, [nom]);
  useEffect(() => {
    localStorage.setItem("in_email", email);
  }, [email]);
  useEffect(() => {
    localStorage.setItem("in_tel", telephone);
  }, [telephone]);
  useEffect(() => {
    localStorage.setItem("in_addr", address);
  }, [address]);
  useEffect(() => {
    localStorage.setItem("in_nat", nationality);
  }, [nationality]);
  ...
  const nationalityError = useMemo(() => {
    if (!touched.nationality) return "";
    if (!nationality.trim()) return "La nationalité est obligatoire.";
    if (nationality.trim().length < 2)
      return "Indiquez une nationalité valide.";
    return "";
  }, [nationality, touched.nationality]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      ["in_nom", "in_email", "in_tel", "in_addr", "in_nat"].forEach((k) =>
      ...
      if (error?.response?.data?.details?.fieldErrors) {
        const msgs = Object.values(error.response.data.details.fieldErrors)
          .flat()
          .join(" | ");
          ...
          <div className="in-card-img" style={s.cardImg}>
            ...
            <div className="in-card-img-content" style={s.cardImgContent}>
              ...
              <p style={s.cardImgText}>
                Créez votre compte pour accéder à une gestion moderne, fluide et
                intelligente.
              </p>
            </div>
          </div>
            ...
            <form onSubmit={handleSubmit} style={s.form}>
              ...
              <div style={s.fieldGroup}>
                ...
                {touched.address && addressError && (
                  <p style={s.fieldError}>{addressError}</p>
                )}
              </div>
              ...
              <div style={s.fieldGroup}>
                ...
                <select
                  className={`in-doc-select${touched.nationality && nationalityError ? " in-input-error" : ""}`}
                ...
                {touched.nationality && nationalityError && (
                  <p style={s.fieldError}>{nationalityError}</p>
                )}
              </div>
                ...
                <div style={s.fieldGroup}>
                  ...
                  <div className="in-doc-row">
                    ...
                    <input
                      ...
                      placeholder={
                        cinType === "TN" ? "12345678" : "Numéro identité"
                      }
              ...
              {touched.cin && cinError && (
                <p style={{ ...s.fieldError, marginTop: "-10px" }}>
                  {cinError}
                </p>
              )}
              ...
              {passportNumber.trim() && (
                ...
                >
                  {selectedPassport.flag} Numéro complet :{" "}
                  {selectedPassport.prefix}-
...
const s = {
  ...
  cardImgOverlay: {
    ...
    inset: 0,
    background:
      "linear-gradient(to bottom, rgba(0,0,0,0) 35%, rgba(10,35,100,0.85) 100%)",
  },
  cardImgContent: {
    position: "relative",
    zIndex: 2,
